...
function ChartTooltipContent({
  ...
  const tooltipLabel = React.useMemo(() => {
    ...
    const [item] = payload;
    const itemKey = item?.dataKey ?? item?.name;
    const key = labelKey ?? (typeof itemKey === "string" ? itemKey : "value");
    ...
    >
      ...
      <div className="grid gap-1.5">
        {payload.map((item, index) => {
          const itemKey = item.name ?? item.dataKey;
          const key = nameKey ?? (typeof itemKey === "string" ? itemKey : "value");
          const itemConfig = getPayloadConfigFromPayload(config, item, key);
          const indicatorColor = (color || item.payload?.["fill"] || item.color) as
            string | undefined;
...
function ChartLegendContent({
  ...
  return (
    ...
    >
      {payload.map((item, index) => {
        const itemKey = item.dataKey ?? item.value;
        const key = nameKey ?? (typeof itemKey === "string" ? itemKey : "value");
        const itemConfig = getPayloadConfigFromPayload(config, item, key);
